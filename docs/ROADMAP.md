...
**State on 2026-09-22:** lesson slice Tasks 1-3 done (`lib/shared/models/`
lesson domain, `assets/lessons/` content, `lib/learning/lesson_{repository,
asset_source,repository_impl}.dart` + `test/learning/` and
`test/fakes/fake_asset_bundle.dart`). Folder layout decided: flat feature
folders under `lib/` (see Decisions log). Next: Task 4 (`lib/progress/`) and
Task 6 (router shell), independent; then Task 5 providers.
...
|-----------|-------|--------|
| 1 — Architecture holds | Tile model + widget, learning vertical slice from JSON asset, in-memory progress, shell routes, tests | in progress: tile model + widget + tests done 2026-09-18 (`b105eb7`); lesson slice plan approved 2026-09-18, Tasks 1-3 (lesson domain, unit 1 asset, `LessonRepository` + asset source + impl in `lib/learning/`) done 2026-09-22, Tasks 4 and 6 next (independent) |
