...
**Milestone 6 closed 2026-09-25 (`5b4e373`).** Seven units (0–6), all four
block types, retry until right, CC0 tile art, README ready for grading
(quick start in three steps with a shared App Check debug token — revoke it
after grading). Known leftover: Google
sign-in on the emulator shows a cosmetic « missing initial state » page after
succeeding (old emulator Chrome). No milestone planned next.
...
| 6 — Content + polish | CC0 tile art (FluffyStuff) via `flutter_svg`, drill (« find in the hand ») + interactive (« complete the group ») blocks, units 2–6 drafted by Claude and reviewed by the author, a content validation test | done 2026-09-25 (`5b4e373`): plan [docs/plans/2026-09-25-content-polish.md](plans/2026-09-25-content-polish.md) + `.tasks.json`, Tasks 0–15 done. CC0 tile art (FluffyStuff) via `flutter_svg`; drill + interactive blocks; `lesson_screen.dart` split into `lib/learning/widgets/`; retry until right (« Réessayer », answer never revealed, Sensei on quiz and drill misses never names it, `TileState.highlighted` removed); seven units drafted by Claude and approved by the author (new unit 0 « Déchiffrer les tuiles », unit 1 renamed « Familles et suites »); content validation test; README reorganised (quick start, requirements map, structure) with emulator guides for macOS, Linux and WSL; shared App Check debug token via `dart_defines.json`. Suite 376. Accessibility pass discarded by the author |
| Grading fixes | README self-hosted API rationale, lesson verdict moved into the notifier, gallery pick in the scanner, convention findings cleared | plan 2026-09-25: [docs/plans/2026-09-25-grading-fixes.md](plans/2026-09-25-grading-fixes.md) + `.tasks.json`; done 2026-09-25 (`d3a6c29`): Tasks 0–5 done. README explains the self-hosted API; lesson verdict, Sensei request and lesson end in the notifier (widgets only read); gallery pick in the scanner; `app.dart` → `tenpai_app.dart`; checker exemptions, conventions 0 findings; suite 388; emulator + airplane-mode round ok |
